...
.sales-wrap .room-cell {{
  min-height: 70px; border-radius: 6px; padding: 6px 4px;
  display: flex; flex-direction: column; justify-content: center; align-items: center;
  cursor: default; color: #fff; font-size: 13px;
...
.sales-wrap .room-cell .room-num  {{ font-weight: 700; font-size: 13px; }}
.sales-wrap .room-cell .room-area {{ font-size: 11px; opacity: .9; margin-top: 2px; }}
...
// ===================================================================
// 15 种状态色（与图例、CSS 类名一一对应）
// ===================================================================
const STATUS_DETAIL_MAP = {{
  1:  {{ cls: 's-presell-on',  label: '预售可售'   }}, // 兜底
  2:  {{ cls: 's-presell-on',  label: '预售可售'   }},
  3:  {{ cls: 's-presell-on',  label: '预售可售'   }},
  4:  {{ cls: 's-sold',        label: '已签约'     }},
  5:  {{ cls: 's-restricted',  label: '限制销售'   }},
  6:  {{ cls: 's-confirm-on',  label: '确权可售'   }},
  7:  {{ cls: 's-confirm-off', label: '确权不可售' }},
  8:  {{ cls: 's-transferred', label: '已过户'     }},
  9:  {{ cls: 's-frozen',      label: '强制冻结'   }},
  10: {{ cls: 's-pledged',     label: '抵押'       }},
  11: {{ cls: 's-sealed',      label: '查封'       }},
  12: {{ cls: 's-relocated',   label: '回迁'       }},
  13: {{ cls: 's-self-use',    label: '自用'       }},
  14: {{ cls: 's-public',      label: '公建配套'   }},
  15: {{ cls: 's-direct-mgmt', label: '直管'       }},
  16: {{ cls: 's-divided',     label: '分成'       }},
  17: {{ cls: 's-filed',       label: '已备案'     }},
  18: {{ cls: 's-not-included',label: '未纳入'     }},
  19: {{ cls: 's-presell-off', label: '不可销售'   }},
}};
...
// 静态图例：严格对应用户上传图片的两行布局（15 种状态色）
const LEGEND_ROWS = [
  // 第一行（用户图片第一行）
  [
    {{ cls: 's-presell-off',  label: '不可销售'   }},
    {{ cls: 's-presell-on',   label: '预售可售'   }},
  ...
  // 第二行（用户图片第二行）
  [
    {{ cls: 's-not-included', label: '未纳入'     }},
    {{ cls: 's-relocated',    label: '回迁'       }},
    {{ cls: 's-self-use',     label: '自用'       }},
    {{ cls: 's-public',       label: '公建配套'   }},
    {{ cls: 's-direct-mgmt',  label: '直管'       }},
    {{ cls: 's-divided',      label: '分成'       }},
    {{ cls: 's-sealed',       label: '查封'       }},
    {{ cls: 's-sold',         label: '已签约'     }},
    {{ cls: 's-filed',        label: '已备案'     }},
  ],
];
  ...
  const html = LEGEND_ROWS.map(row => `
    <div class="legend-row">${{row.map(it => `
      <div class="legend-item">
        <span class="legend-color ${{it.cls}}"></span><span>${{it.label}}</span>
      </div>
    `).join('')}}</div>
  `).join('');
...
// 状态判定：优先根据阳光家缘 status 字段映射到 15 种颜色；
// preSellStatus/status/pactStatus/closed/pledgeStatus 等字段联合判断
function decideStatus(r) {{
  // 强制冻结
  if (r.closed === 1 && r.preSellStatus === 1 && (r.status === 9 || r.pactStatus === 9)) {{
    return STATUS_DETAIL_MAP[9];
  }}
  // 查封
  if (r.sealed === 1 || r.status === 11) return STATUS_DETAIL_MAP[11];
  // 抵押
  if (r.pledgeStatus === 2) return STATUS_DETAIL_MAP[10];
  // 不可销售（官方 closed=1）
  if (r.closed === 1) return STATUS_DETAIL_MAP[19];
  // 限制销售
  if (r.restricted === 1 || r.status === 5) return STATUS_DETAIL_MAP[5];
  // 已过户
  if (r.status === 8) return STATUS_DETAIL_MAP[8];
  // 确权不可售
  if (r.status === 7) return STATUS_DETAIL_MAP[7];
  // 确权可售
  if (r.status === 6) return STATUS_DETAIL_MAP[6];
  // 已签约
  if (r.preSellStatus === 1 || r.status === 4) return STATUS_DETAIL_MAP[4];
  // 回迁/自用/公建/直管/分成/已备案/未纳入（按 status 二次判定）
  if (r.status >= 12 && r.status <= 18) return STATUS_DETAIL_MAP[r.status];
  // 默认：预售可售
  return STATUS_DETAIL_MAP[3];
}}
...
function renderSalesTable(unit) {{
  ...
  // 收集所有房号作为列；按 unitNum 字符串排序
  const colSet = new Set();
  sortedFloors.forEach(f => (f.groupData || []).forEach(r => colSet.add(r.unitNum)));
  const cols = Array.from(colSet).sort();

  // 列数 = 该楼栋披露的「户数」（所有楼层房号的并集），由 JS 动态写入 grid-template-columns
  const colCount = Math.max(cols.length, 1);
  // 渲染：第 1 列为楼层号，其后每列对应一个房号（有几户就有几列）
