...
// 符号定义：sym=显示字符，trigger=判定函数，order=优先级（数字越小越靠前）
const MARK_DEFS = [
  {{ sym: '■',  order: 1,  label: '已网签/已签约', test: r => r.preSellStatus === 1 || r.status === 4 || r.pactStatus === 4 }},
  {{ sym: '◆',  order: 2,  label: '已备案',        test: r => r.status === 4 || r.pactStatus === 5 || r.filed === 1 }},
  {{ sym: '‡',  order: 3,  label: '抵押',          test: r => r.pledgeStatus === 2 || r.status === 10 }},
  {{ sym: '★',  order: 4,  label: '公建配套',      test: r => r.commonMatch === 1 || r.status === 14 }},
  {{ sym: '☆',  order: 5,  label: '回迁',          test: r => r.backMove === 1 || r.status === 12 }},
  {{ sym: '◻',  order: 6,  label: '自用',          test: r => r.useself === 1 || r.status === 13 }},
  {{ sym: '△',  order: 7,  label: '未纳入',        test: r => r.status === 18 }},
  {{ sym: '▲',  order: 8,  label: '直管',          test: r => r.directly === 1 || r.status === 15 }},
  {{ sym: '✚',  order: 9,  label: '分成',          test: r => r.divide === 1 || r.status === 16 }},
  {{ sym: '✕',  order: 10, label: '查封',          test: r => r.sealed === 1 || r.status === 11 }},
];
...
function decideStatus(r) {{
  let cls = 's-presell-on';   // 默认：预售可售（亮绿）
  let label = '预售可售';
  // 强制冻结（最高优先级）
  ...
  // 其余（含 status 1/2/3、preSellStatus===1、status===5 等）→ 默认亮绿
  // 符号收集（按优先级排序）
