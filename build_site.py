...
// ===================================================================
const COLOR_MAP = {{
  's-presell-off':  {{ color: '#E53935', label: '不可销售'    }},
  's-confirm-off':  {{ color: '#C62828', label: '确权不可售'  }},
  's-presell-on':   {{ color: '#7CB342', label: '预售可售'    }},
...
const MARK_DEFS = [
  {{ sym: '⊙',  order: 1,  label: '已签约',   test: r => r.pactStatus === 5 || r.pactStatus === 4 || r.status === 4 }},
...
// 状态判定：颜色 = 销售状态（可售/不可售/已过户/冻结/确权可售/确权不可售）
//         符号 = 该户的具体附加情况（已签约/已备案/抵押/查封/回迁/自用/公建/直管/分成/未纳入）
// 关键规则（与阳光家缘官方一致）：已签约/已备案/抵押/查封/回迁/自用/公建/直管/分成/未纳入 等
// 都属于"不可销售"，统一红色，符号区分原因。
function decideStatus(r) {{
  ...
  // 5. 已签约 → 不可销售（红）
  //    关键：pactStatus===5 表示已签合同（最常见的已签约），pactStatus===4 也表示已签
  //    preSellStatus===1 + status===4 也可能是已签
  else if (r.pactStatus === 5 || r.pactStatus === 4 || r.status === 4) {{ cls = 's-presell-off'; label = '已签约（不可销售）'; }}
  // 6. 已备案 → 不可销售（红）
  else if (r.status === 17 || r.filed === 1) {{ cls = 's-presell-off'; label = '已备案（不可销售）'; }}
  // 7. 查封 → 不可销售（红）
  else if (r.sealed === 1 || r.status === 11) {{ cls = 's-presell-off'; label = '查封（不可销售）'; }}
  // 8. 抵押 → 不可销售（红）
  else if (r.pledgeStatus === 2 || r.status === 10) {{ cls = 's-presell-off'; label = '抵押（不可销售）'; }}
  // 9. 特殊用途（回迁/自用/公建/直管/分成/未纳入）→ 不可销售（红）
  else if (r.backMove === 1 || r.status === 12) {{ cls = 's-presell-off'; label = '回迁房（不可销售）'; }}
  else if (r.useself === 1 || r.status === 13) {{ cls = 's-presell-off'; label = '自用房（不可销售）'; }}
  else if (r.commonMatch === 1 || r.status === 14) {{ cls = 's-presell-off'; label = '公建配套（不可销售）'; }}
  else if (r.directly === 1 || r.status === 15) {{ cls = 's-presell-off'; label = '直管房（不可销售）'; }}
  else if (r.divide === 1 || r.status === 16) {{ cls = 's-presell-off'; label = '分成（不可销售）'; }}
  else if (r.status === 18) {{ cls = 's-presell-off'; label = '未纳入预售（不可销售）'; }}
  // 10. 限制销售 → 红
  else if (r.status === 5 || r.restricted === 1) {{ cls = 's-presell-off'; label = '限制销售'; }}
  // 11. 官方 closed=1 兜底 → 红
  else if (r.closed === 1) {{ cls = 's-presell-off'; label = '不可销售'; }}
  // 12. 默认（status 1/2/3）→ 亮绿
  // 符号收集（按优先级）
